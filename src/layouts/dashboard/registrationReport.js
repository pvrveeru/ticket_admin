...
function RegistrationReport() {
  ...
  const handleRegistrationSearch = useCallback(async () => {
    ...
    const url = `Report/GetRegistrationCountByDate?fromDate=${formattedStartDate}&toDate=${formattedEndDate}`;
    ...
    try {
      ...
      if (response.status === 200) {
        setRdata(response.data.registrationsCount || []); // Set rdata from response.data
      } else {
        setError("Failed to fetch reports. Please try again.");
      }
    } catch (error) {
      setError("Error fetching reports. Please check your connection.");
    } finally {
      setLoading(false);
    }
  ...
  const exportregistrationCSV = () => {
    ...
    saveAs(blob, "registrationreport.csv");
  };

  const convertRegistrationToCSV = (data) => {
    if (data.length === 0) return "";

    const header = ["Date", "Count"].join(",");
    const rows = data.map((row) => [row.date, row.count].join(","));

    return `${header}\n${rows.join("\n")}`;
  };

  const getTotalCount = () => {
    return rdata.reduce((total, row) => total + row.count, 0);
  };
            ...
            <Card style={{ paddingBottom: "20px" }}>
              ...
              >
                <MDTypography variant="h6" color="white">
                  <h2>Registration Count Reports</h2>
                </MDTypography>
              </MDBox>
              <MDBox pt={3} px={2}>
                      ...
                      {rdata.length > 0 ? (
                        <>
                          <MDBox mt={2} mb={2}>
                            ...
                            >
                              Total Registrations: <b>{getTotalCount()}</b>
                            </span>
                          </MDBox>
